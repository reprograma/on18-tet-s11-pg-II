...
const obterBoletim = async (req, res) => {
  //  // para casa
   const { boletim } = req.params

   try {
    const notasparaoBoletim = await db()
    const alunasBoletim = notasparaoBoletim.filter(alunaAtual=> alunaAtual.boletim == boletim)
    if(alunasBoletim.lenght == 0){
      return res.status(404).json({ message: "Nenhuma aluna encontrada para a turma de ${boletim}."})
    }
    let notasdasAlunas;
    let media;
    let alunasFiltradas = [];
    alunasBoletim.array.forEach(aluna => {
      notasdasAlunas = Object.values(aluna.notas)
      media = (notasdasAlunas.reduce((acumulador, nota)=> Number(acumulador) + Number(nota))) / 5
      if (media >= 6) {
        situacao = "APROVADA"
      } else if (media >= 5) {
...
const deletarAluna = async (req, res) => {
  ...
  try {
    ...
    // const indice = alunas.indexOf(alunasEncontradas)
